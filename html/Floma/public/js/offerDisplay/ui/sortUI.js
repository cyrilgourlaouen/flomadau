export function setupSortUI(sortManager, displayManager) {
  ...
  const sortDesktopButton = document.querySelector(
    '#offer-sort-desktop-button'
  );
  const sortDesktopOptions = document.querySelector(
    '#offer-sort-desktop-options'
  );
  const sortDesktopOptionItems = document.querySelectorAll(
    '#offer-sort-desktop-options p'
  );
  const selectedSortDesktopLabel = document.getElementById(
    'selected-sort-desktop-label'
  );

  // --- CONFIGURATION MOBILE ---
  const sortMobileButton = document.querySelector('#offer-sort-mobile-button');
  const sortMobileOptions = document.querySelector(
    '#offer-sort-mobile-options'
  );
  const sortMobileOptionItems = document.querySelectorAll(
    '#offer-sort-mobile-options p'
  );
  const selectedSortMobileLabel = document.getElementById(
    'selected-sort-mobile-label'
  );
  ...
  function handleSortSelection(sortType, source) {
    ...
    if (selectedSortDesktopLabel) {
      const selectedOption = Array.from(sortDesktopOptionItems).find(
        (opt) => (opt.id || opt.getAttribute('data-sort-type')) === sortType
      );
      if (selectedOption) {
    ...
    if (selectedSortMobileLabel) {
      const selectedOption = Array.from(sortMobileOptionItems).find(
        (opt) => (opt.id || opt.getAttribute('data-sort-type')) === sortType
      );
      if (selectedOption) {
  ...
  if (sortMobileButton && sortMobileOptions) {
    ...
    sortMobileButton.addEventListener('click', (e) => {
      ...
      sortMobileOptions.style.display =
        sortMobileOptions.style.display === 'none' ? 'block' : 'none';
    });
